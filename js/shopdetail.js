...
//放大镜插件
//     $(".goods_pic").jqueryzoom({
//         xzoom: 470, //放大图的宽度(默认是 200)
//         yzoom: 430, //放大图的高度(默认是 200)
...
//     });
    ...
    $gp.mousemove(function(e){
        ...
        // alert(e.pageX-_x)
        // alert(e.pageY-_y)
        // alert("$fl.width()/2"+$fl.width()/2)
        // alert("$fl.height()/2"+$fl.height()/2)($gp.width()-$fl.width())
        var zx=parseInt($gp.width())-parseInt($fl.width());
        var tx=parseInt($gp.height())-parseInt($fl.height());
        // alert(tx)
        if(l<0){
            l=0;
...
var xy_index=0;
   ...
   $(".shop_size span").click(function(){
       ...
       xy_index=$(this).index();
       $('.bottom_buy_middle_left span').eq(xy_index-1).css({"border":"1px solid red"}).siblings().css("border-color","#ccc");
       $('.bottom_buy_middle_left')
       $('.bottom_buy_middle_left span').eq(xy_index-1).find("i").show();
       $('.bottom_buy_middle_left span').eq(xy_index-1).find("i").hide();
       $(this).find("i").show();
...
$(".shop_number_cut").click(function(){
   ...
   var $addcart=$(".goods_addtocart")
    //首先需要判断几个选项是否有选中
    var strx='<div class="creatshopdiv" style="width:100px;height:100px;background: lightpink;text-align: center;position: absolute;left: 10px;top:0;font-size: 22px;color: white;line-height: 50px;overflow: hidden">加入购物车成功</div>';
    ...
    $addcart.click(function(){
        ...
        if(flag1&&flag2){
         ...
         setCookie("shop_price",$priceshop,d);
            ...
            setTimeout(function () {
                $(".shop_size_compare").html(strx);
                $(".creatshopdiv").fadeIn(800,function () {
                  setTimeout(function () {
                      $(".creatshopdiv").animate({"top":"-500px","left":"430px","width":"0px","height":"0px","font-size":"0px"},function () {
                          $(".creatshopdiv").fadeOut(800);
                      })
